...
function buildComparisonData(sources, percKey, weekRange) {
  return weekRange.map(w => {
    const row = { week: w };
    sources.forEach(s => {
      row[s.short] = getVal(s, w, percKey);
    });
    return row;
...
const css = {
  ...
  tab: (active) => ({ padding: "8px 16px", borderRadius: 8, border: `1px solid ${active ? palette.accent2 : palette.cardBorder}`, background: active ? palette.accent2 + "22" : palette.card, color: active ? "#fff" : palette.textMuted, cursor: "pointer", fontSize: 13, fontWeight: active ? 600 : 400, transition: "all .2s" }),
  ...
  legendItem: (color) => ({ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: palette.textMuted }),
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }}>
        ...
        <Bar dataKey="diff" name="Разница (г)" fill={a.color} radius={[3,3,0,0]}
          fillOpacity={0.8}
          ...
          {weekRange.map(w => {
            ...
            return (
              <tr key={w}>
                ...
                {vals.map((v, i) => <td key={i} style={css.td}>{v ?? "—"}</td>)}
...
export default function App() {
  ...
  return (
    <div style={css.page}>
      ...
      <div style={css.tabs}>
        {GROUPS.map(g => (
          <button key={g.id} style={css.tab(groupId === g.id)} onClick={() => setGroupId(g.id)}>
            {g.label}
          </button>
        ))}
      </div>
      ...
      <div style={css.percTabs}>
        {PERCENTILES.map(p => (
          <button key={p} style={css.percTab(perc === p)} onClick={() => setPerc(p)}>
            {PERC_LABELS[p]}
          </button>
        ))}
      </div>
      ...
      <div style={css.legend}>
        {group.sources.map(s => (
          <div key={s.short} style={css.legendItem(s.color)}>
      ...
      <div style={css.card}>
        <div style={css.cardTitle}>
          {PERC_LABELS[perc]} перцентиль — абсолютные значения (г)
        </div>
        <CompChart sources={group.sources} percKey={perc} weekRange={weeks} />
      </div>

      {/* Diff chart for pair comparisons */}
      {isPair && (
        <div style={css.card}>
          <div style={css.cardTitle}>
            Разница: {group.sources[0].short} − {group.sources[1].short} ({PERC_LABELS[perc]})
          </div>
      ...
      {/* All percentiles overview for "All 3" */}
      {groupId === "all" && (
        <div style={css.card}>
          <div style={css.cardTitle}>Все перцентили — медиана (50-й) по неделям</div>
          <div style={css.scrollBox}>
            <table style={css.table}>
              <thead>
                <tr>
              ...
              <tbody>
                {weeks.map(w => {
                  ...
                  return (
                    <tr key={w}>
                      <td style={{ ...css.td, ...css.tdFirst }}>{w}</td>
                      <td style={css.td}>{vh ?? "—"}</td>
                      <td style={css.td}>{v16 ?? "—"}</td>
                      <td style={css.td}>{v20 ?? "—"}</td>
                      <td style={{ ...css.td, color: "#f39c12" }}>{maxD ?? "—"}</td>
